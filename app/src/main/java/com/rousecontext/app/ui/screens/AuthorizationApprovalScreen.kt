...
fun AuthorizationApprovalScreen(
    ...
    Surface(modifier = Modifier.fillMaxSize()) {
        if (pendingRequests.isEmpty()) {
            ...
            // Clamp index in case list shrinks after approve/deny
            val safeIndex = currentIndex.coerceIn(0, pendingRequests.lastIndex)
            val request = pendingRequests[safeIndex]
            ...
            Column(
                ...
                // Counter when multiple pending
                if (pendingRequests.size > 1) {
                    Text(
                        text = "${safeIndex + 1} of ${pendingRequests.size}",
