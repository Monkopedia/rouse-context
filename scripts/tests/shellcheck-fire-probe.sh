#!/usr/bin/env bash
# TEMPORARY fire-proof probe for #592 -- reverted in the commit that follows.
# A copy of check-roborazzi-step-order.sh with one deliberate SC2155.
#
...
index_of() {
  # DELIBERATE DEFECT (temporary #592 fire-proof, reverted in the next commit):
  # `local` always succeeds, so grep's exit status is discarded here. This is
  # the masked-exit-status shape these gates are built out of, and it is what
  # the shellcheck step must catch -- SC2155.
  local hit=$(printf '%s\n' "$steps" | grep -nF -- "$1" | head -1)
  printf '%s\n' "$hit" | cut -d: -f1
}
