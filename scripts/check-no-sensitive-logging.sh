...
# Kotlin comment does not fail the build. The anchoring lives in
# scripts/lib/gate-filters.sh, which documents why it is a CONTENT-field filter
# and what the unanchored form swallowed here: `grep -v ':[[:space:]]*\*'` also
# matched `: *` anywhere in a log string and threw away real violations (#577).
# `grep -v` exits 1 when it emits nothing, which for this gate is the good case,
# so the status is discarded -- via the same `set +e` bracket the detection grep
# above uses, rather than the trailing `|| true` this line carried before, which
# would put a function in an `||` condition and turn set -e off inside it
# (SC2310).
set +e
